...
if(realpath(__FILE__) == realpath($_SERVER['SCRIPT_FILENAME']))
	exit;
if(!defined('GARDEN_BLOG_CLASS')){
	define('GARDEN_BLOG_CLASS',true);
	class clsCloneBlog{
		public $entries;
		public $gardens;
		
		public function clsGardenBlog($garden){
			global $db;
			$this->entries = array();
			$this->gardens = $garden;
			$clone_start_date = strtotime($garden->start_date);
			$now = strtotime($clone->end_date);
			$day_step = 60*60*24;
			for($i = $now; $i >= $clone_start_date; $i -= $day_step){
				$entry = clsGardenBlogEntry::loadEntryDay($db,date("Y-m-d",$i),$garden);
				if($entry != NULL)
		...
		public function this_week(){
			return new clsGardenBlogWeek(date("Y-m-d",time()),$this->entries);
		}
		public function previous_week($weeks_ago = 1){
			return new clsGardenBlogWeek(date("Y-m-d",time()-(60*60*24*7*$weeks_ago)),$this->entries);
		}
		
		public static function loadBlog($db,$clone){
			$blog = new clsGardenBlog($clone);
			return $blog;
		}
	}
	class clsGardenBlogWeek{
		public $date_start;
		public $date_end;
		public $entries;
		public function clsGardenBlogWeek($date,$entries){
	...
	class clsGardenBlogEntry{
		...
		public function clsGardenBlogEntry($db,$date,$garden){
		...
		public static function loadEntryDay($db,$date,$garden){
			$entry = new clsGardenBlogEntry($db,$date,$garden);
